              ...
              git clone --recurse-submodules https://github.com/Area69Lab/ErfanGSIs.git gsi && git clone https://github.com/akhilnarang/scripts && sudo bash scripts/setup/android_build_env.sh && sudo bash gsi/setup.sh &&sudo apt update && sudo apt upgrade -y
